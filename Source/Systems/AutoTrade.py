...
def is_trend_initial(candles, min_body_size=0.003, min_breakout_ratio=0.005):
    ...
    # ヒゲ比率チェック(USE_STDEVが有効な場合はスキップ有効)
    if USE_STDEV == 1: 
        if upper_wick_3 > body_3 * 2.5:
            logging.info(
...
YDAY_UP_STOP_1 = config["YDAY_UP_STOP_1"]
# 夜間判定関数
...
def close_order(position_id, size, side):
    ...
    try:
        ...
        halt = profit_lock_check(API_KEY, API_SECRET, SYMBOL, YDAY_UP_STOP_1)
...
async def monitor_trend(stop_event, short_period=6, long_period=13, interval_sec=3, shared_state=None):
    ...
    while not stop_event.is_set():
        ...
        last5 = list(price_buffer)[-5:]

        if len(last5) >= 2:
            stdev5 = statistics.stdev(last5)
        else:
            logging.info("stdev5:データ蓄積待ち")
            stdev5 = 0.0
